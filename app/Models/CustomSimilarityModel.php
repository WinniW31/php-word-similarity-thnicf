 ...
 * value of 0 means that the strings are entirely different.
 * A value of 1 means that the strings are identical.
 ...
 *
 * You can add an optional argument $limit (default 0) that gives the minimum similarity the two strings must satisfy.
 * similarity stops analyzing the string as soon as the result drops below the given limit,
 * in which case the result will be invalid but lower than the given $limit.
 * You can use this to speed up the common case of searching for the most similar
 * string from a set by specifing the maximum similarity found so far.
 *
 */


define("SNAKE_LIMIT",	200);
define("INT_MAX" , 2147483647);

class CustomSimilarityModel extends Model
{
  ...
  function fstrcmp ($string1, $length1, $string2, $length2, $minimum = 0)
  {
    ...
    $checkstrPOS = $this->findStrPOS($this->string[0]["data"] , $this->string[1]["data"]);

    if($checkstrPOS === false){
      /* The result is
  ...
  public function findStrPOS($s1 , $s2){
    $result = str_contains($s1, $s2);
    if($result !== false){
      return true;
    }else {
      return false;
